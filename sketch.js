...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        // Vorm: a(x + b) + cx = d(x + e) + fx
        // Of simpeler: a(x ± b) = c(x ± d)
        
        let variant = floor(random(3)); // 3 verschillende types
        let text = "";
        let answer = 0;
        
        if (variant === 0) {
            // Type 1: a(x + b) = c(x + d)
            // Uitwerken: ax + ab = cx + cd
            // ax - cx = cd - ab
            // x(a - c) = cd - ab
            // x = (cd - ab) / (a - c)
            
            let a = floor(random(2, 8));
            let b = floor(random(-10, 10));
            let c = floor(random(-6, 6));
            let d = floor(random(-10, 10));
            
            // Zorg dat getallen niet 0 zijn en a ≠ c
            if (b === 0) b = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (d === 0) d = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (a === c) c = c + 1;
            if (c === 0) c = floor(random(2, 6));
            
            // Bereken antwoord
            answer = (c * d - a * b) / (a - c);
            
            // Check geldigheid
            if (!Number.isInteger(answer) || Math.abs(answer) > 20) {
                i--;
                continue;
            }
            
            // Maak tekst
            let leftSide = (a === 1 ? "" : a) + "(x" + (b >= 0 ? " + " + b : " - " + Math.abs(b)) + ")";
            let rightSide = (c === 1 ? "" : (c === -1 ? "-" : c)) + "(x" + (d >= 0 ? " + " + d : " - " + Math.abs(d)) + ")";
            
            text = leftSide + " = " + rightSide;
            
        } else if (variant === 1) {
            // Type 2: a(x + b) + cx = d(x + e)
            // Uitwerken: ax + ab + cx = dx + de
            // ax + cx - dx = de - ab
            // x(a + c - d) = de - ab
            // x = (de - ab) / (a + c - d)
            ...
            let b = floor(random(-10, 10));
            let c = floor(random(-5, 5));
            let d = floor(random(2, 6));
            let e = floor(random(-10, 10));
            
            // Zorg dat getallen niet 0 zijn
            if (b === 0) b = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (e === 0) e = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (c === 0) c = floor(random(1, 5)) * (random() > 0.5 ? 1 : -1);
            if (a + c === d) c = c + 1;
            
            // Bereken antwoord
            answer = (d * e - a * b) / (a + c - d);
            ...
            if (c > 0) {
                ...
                leftSide += " - " + (c === -1 ? "" : Math.abs(c)) + "x";
            }
            
            let rightSide = (d === 1 ? "" : d) + "(x" + (e >= 0 ? " + " + e : " - " + Math.abs(e)) + ")";
            
            text = leftSide + " = " + rightSide;
            
        } else {
            // Type 3: a(x + b) = c(dx + e) + fx
            // Uitwerken: ax + ab = cdx + ce + fx
            // ax - cdx - fx = ce - ab
            // x(a - cd - f) = ce - ab
            // x = (ce - ab) / (a - cd - f)
            ...
            let a = floor(random(2, 6));
            let b = floor(random(-10, 10));
            let c = floor(random(2, 5));
            let d = floor(random(2, 4));
            let e = floor(random(-10, 10));
            let f = floor(random(-5, 5));
            
            // Zorg dat getallen niet 0 zijn
            if (b === 0) b = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (e === 0) e = floor(random(1, 8)) * (random() > 0.5 ? 1 : -1);
            if (f === 0) f = floor(random(1, 5)) * (random() > 0.5 ? 1 : -1);
            if (a === c * d + f) f = f + 1;
            
            // Bereken antwoord
            answer = (c * e - a * b) / (a - c * d - f);
            ...
            let rightSide = (c === 1 ? "" : c) + "(" + (d === 1 ? "" : d) + "x" + (e >= 0 ? " + " + e : " - " + Math.abs(e)) + ")";
            if (f > 0) {
                rightSide += " + " + (f === 1 ? "" : f) + "x";
            } else if (f < 0) {
                rightSide += " - " + (f === -1 ? "" : Math.abs(f)) + "x";
            }
...
function drawBlock(block) {
    ...
    let txt = block.text;
    let maxWidth = CELL_SIZE - 20;  // Beschikbare breedte
    
    // Vaste tekstgrootte voor alle blokjes
textSize(12);  // ← VERANDER DIT GETAL voor andere grootte
    
    // Meet hoe breed de tekst is
    let txtWidth = textWidth(txt);  // ← Deze functie meet de breedte in pixels!
    
    // ALLEEN splitsen als tekst te breed is EN er een "=" in zit
    if (txtWidth > maxWidth && txt.includes(" = ")) {
        // Tekst past niet op 1 regel → splits bij "="
        let parts = txt.split(" = ");
        ...
        if (parts.length === 2) {
            ...
            // Kan niet splitsen, teken gewoon op 1 regel (wordt afgesneden)
            text(txt, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
        }
    } else {
        // Tekst past wel op 1 regel → normaal tekenen
        text(txt, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
    }
}
